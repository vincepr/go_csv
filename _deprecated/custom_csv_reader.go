...
import (
	...
	"github.com/vincepr/go_csv/deprecated/csv"
...
func main() {
    ...
    println("running on width:",width,"height:",height, "filepath:",path)

    // read the file
    table, err := csv_custom.Read(path)
    if err != nil{
        fmt.Printf("%s\n",err)
        return
    }
    // printOut the table to the terminal
    csv_custom.Print(table)
}
